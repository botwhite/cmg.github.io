...
// File: contracts\MinerGlobal.sol
...
contract MinerGlobal is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    IERC20 public token_BUSD;
	address erctoken = 0x4E93Fb4c823754943100632dF2bD1EFf66487829; 
    ...
    // Price of each token
    uint256 public initial_price = 1000 ether;
    uint256 public price;

    // Maximum limit of tokens that can ever exist
    uint256 public constant MAX_SUPPLY = 1000;
    uint256 public constant MAX_PRESALE_SUPPLY = 0;
    uint256 public constant MAX_MINT_PER_TX = 20;

    // The base link that leads to the image / video of the token
    string public baseTokenURI = "http://157.245.126.14/";

    // Team addresses for withdrawals
    address public a1 = 0x6a7fD2778494f6ADd4D53CD17d0EEa754b58D80b;
    address public a2 = 0x9cCA2B481a0D9fae739C443f79E3095C9b232EA5;
   

    // List of addresses that have a number of reserved tokens for whitelist
    mapping (address => uint256) public whitelistReserved;

    constructor () ERC721 ("Miner Global", "MG") {
        price = initial_price;
   ...
   function getBalance() public view returns (uint256) {
    ...
    function mintToken(uint256 _amount) public payable {
        ...
        require( supply + _amount <= MAX_SUPPLY,            "Can't mint more than max supply" );
        //require( msg.value == price * _amount,              "Wrong amount of ETH sent" );
        require(token_BUSD.balanceOf(msg.sender) >= price * _amount);
        token_BUSD.transferFrom(msg.sender,  address(this), price * _amount);
        //token_BUSD.transferFrom(address(msg.sender), address(this), _amount);

        //token_BUSD.transferFrom(msg.sender, address(0), price);
        for(uint256 i; i < _amount; i++){
            _safeMint( msg.sender, supply + i );
        }
    }
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        token_BUSD.transfer(address(a1),  percent * 50);
        token_BUSD.transfer(address(a2),  percent * 50);
